...
class ForgetPassCodeVC: UIViewController {
...
//        navigationController?.pushViewController(vc, animated: true)
    ...
    func forgetPasswordCode(email:String,code:String){
        ...
        NetworkManager.shared.getData(CheckPassModel.self, Requst: .check_forget_code(phone: email, code: code), method: .post, headerType: .unAuthenticated) {[weak self] (Massage, Data, Code) in
            self?.closeIndicator()
            if Code == 200 {
...
//
    ...
    func checkForgetPassword(email:String,type:String){
        openIndicator(title:Constants.PLEASE_WAIT , description: Constants.LOADING_DATA)
        NetworkManager.shared.getData(ForgetPassModel.self, Requst: .resend_forget_code(phone: email, type: type), method: .post, headerType: .unAuthenticated) {[weak self] (Massage, Data, Code) in
            self?.closeIndicator()
            if Code == 200 {
...
extension ForgetPassCodeVC: UITextFieldDelegate{
