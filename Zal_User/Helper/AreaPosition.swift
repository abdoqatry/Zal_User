...
protocol Areaprotocole {
    func passCity2(value : String,Id :Int,code:String)
}

class AreaPositionVC: UIViewController {
...
//        let CityId = String(id)
    ...
    func getPosition(id:String){
        ...
        NetworkManager.shared.getData(CityModel.self, Requst: .areas(city_id: id), method: .get, headerType: .unAuthenticated) {[weak self] (Massage, Data, Code) in
            self?.closeIndicator()
            if Code == 200{
...
//                    if Data?.cities?.count != 0 {
                        var date = Data?.data ?? []
                        let person1: Citi = Citi(id: 0, name: "Select Area".localize, code: "-10", image: "")
...
//                }
    ...
    @objc func Confirm(){
        ...
        print(Name)
        delegetArea?.passCity2(value : Name,Id :id,code:code)
        dismiss(animated: true, completion: nil)
    }
    @objc func diss(){
        delegetArea?.passCity2(value : "",Id :-1,code:"-1")
        dismiss(animated: true, completion: nil)
    }
    func settarget(){
...
extension AreaPositionVC : UIPickerViewDelegate,UIPickerViewDataSource{
